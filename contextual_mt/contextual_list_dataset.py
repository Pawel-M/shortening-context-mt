...
def collate_vectors(
    ...
    vector_size = values[0].size(-1)
    res = values[0].new(batch_size, size, vector_size).fill_(pad_idx)
...
def collate(samples, pad_id, eos_id, sort_by_src=False):
    ...
    src_ctx_size = len(samples[0]['src_context'])
    src_ctx_tokens = [data_utils.collate_tokens(
        [s["src_context"][i] for s in samples],
        pad_id,
        eos_id,
    ) for i in range(src_ctx_size)]
    src_ctx_lengths = [
        torch.LongTensor([s["src_context"][i].ne(pad_id).long().sum() for s in samples])
        for i in range(src_ctx_size)
    ]
    # encode target and target context
    tgt_ctx_size = len(samples[0]['tgt_context'])
    tgt_ctx_tokens = [data_utils.collate_tokens(
        [s["tgt_context"][i] for s in samples],
        pad_id, eos_id, move_eos_to_beginning=True
    ) for i in range(tgt_ctx_size)]
    tgt_ctx_lengths = [
        torch.LongTensor([s["tgt_context"][i].ne(pad_id).long().sum() for s in samples])
        for i in range(tgt_ctx_size)
    ]

    if samples[0].get("cached_src_context", None) is not None:
        cached_src_ctx_size = len(samples[0]['cached_src_context'])
        cached_src_context = [
            collate_vectors([s['cached_src_context'][i] for s in samples], pad_id)
            for i in range(cached_src_ctx_size)
        ]
        cached_src_context_padding_mask = [
            data_utils.collate_tokens([s['cached_src_context_padding_mask'][i] for s in samples], pad_idx=True)
            for i in range(cached_src_ctx_size)
        ]
    else:
    ...
    if sort_by_src:
        ...
        src_ctx_tokens = [s.index_select(0, sort_order) for s in src_ctx_tokens]
        src_ctx_lengths = [s.index_select(0, sort_order) for s in src_ctx_lengths]
        tgt_ctx_tokens = [t.index_select(0, sort_order) for t in tgt_ctx_tokens]
        tgt_ctx_lengths = [t.index_select(0, sort_order) for t in tgt_ctx_lengths]
    ...
    if cached_src_context is not None:
        batch['net_input']['cached_src_context'] = cached_src_context
        batch['net_input']['cached_src_context_padding_mask'] = cached_src_context_padding_mask
    ...
    if samples[0].get("target", None) is not None:
        ...
        if sort_by_src:
            ...
            tgt_ctx_tokens_out = [t.index_select(0, sort_order) for t in tgt_ctx_tokens_out]
...
class ContextualListDataset(FairseqDataset):
    """
    ...
    def __getitem__(self, index):
        ...
        src_ctx_items = list([torch.tensor([src_eos_id]).long() for _ in range(self.src_ctx_size)])
        tgt_ctx_items = list([torch.tensor([tgt_eos_id]).long() for _ in range(self.tgt_ctx_size)])
        ...
        if self.src_ctx_size > 0:
            ...
            for i in range(1, src_context_size + 1):
                ...
                src_ctx_items[self.src_ctx_size - i] = torch.cat([self.src[index - i][:-1], src_eos_id])
        ...
        if self.tgt_ctx_size > 0:
            ...
            for i in range(1, tgt_context_size + 1):
                ...
                tgt_ctx_items[self.tgt_ctx_size - i] = torch.cat([self.tgt[index - i][:-1], tgt_eos_id])
        ...
        sample = {
            "id": index,
            "src_context": src_ctx_items,
            "source": src_item,
            "tgt_context": tgt_ctx_items,
            "target": tgt_item,
        }
    ...
    def __len__(self):
        if self.limit_size is not None and self.limit_size > 0:
            return min(self.limit_size, len(self.src))
